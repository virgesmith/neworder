...
void test_no()
{
  ...
  for (size_t i = 0; i < sizeof(attrs)/sizeof(attrs[0]); ++i)
  {
    ...
    ;
    CHECK(py::extract<bool>(neworder::Callback::eval(format("'%%' in locals()", attrs[i]))()));
  }

  // Check diagnostics consistent
  CHECK(py::extract<bool>(neworder::Callback::eval(format("name() == '%%'", neworder::module_name()))()));
  CHECK(py::extract<bool>(neworder::Callback::eval(format("version() == '%%'", neworder::module_version()))()));
  CHECK(py::extract<bool>(neworder::Callback::eval(format("python() == '%%'", neworder::python_version().c_str()))()));
