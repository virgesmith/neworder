...
class Infection(no.Model):
    def __init__(
        ...
        agents = pd.DataFrame(
            data={
                "node": start_positions,
                "speed": speeds,
                "status": Status.SUSCEPTIBLE,
                "t_infect": no.time.NEVER,
            }
        )
        agents["dest"] = agents["node"].apply(self.__random_next_dest)
        agents["path"] = agents[["node", "dest"]].apply(
    ...
    def __update_position(self) -> None:
        ...
        self.agents["geometry"] = self.agents[["path", "offset"]].apply(
            lambda r: line_interpolate_point(r["path"], r["offset"]), axis=1
        )
    ...
    def __init_visualisation(self) -> tuple[Any, Any]:
        ...
        g = ax.scatter(
            self.agents.geometry.x,
            self.agents.geometry.y,
            color=self.agents.status.apply(lambda c: c.rgba),
            edgecolor="k",
        )
    ...
    def __update_visualisation(self, num_infected, num_immune, num_dead) -> None:
        ...
        self.fig.suptitle(
            f"step {self.timeline.index}: inf={num_infected} imm={num_immune} dead={num_dead} / {len(self.agents)} [q to quit]"
        )
